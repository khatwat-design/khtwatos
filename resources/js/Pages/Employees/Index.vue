...
function approveLate(attendanceId) {
    if (!attendanceId) return;
    router.post(route('attendance.approve-late', { attendance: attendanceId }), {}, {
        preserveScroll: true,
        preserveState: true,
    });
}
...
function openEditModal(employee) {
    ...
    editForm.late_threshold_time = employee.late_threshold_time || '11:00';
                ...
                >
                    ...
                    <div class="flex flex-col gap-2 border-b border-slate-200/70 bg-gradient-to-l from-slate-50 via-white to-brand-50/40 p-4 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div class="flex flex-col items-end gap-1">
                            ...
                            <span
                                v-if="analyticsFor(emp.id)?.attendance?.is_late && !analyticsFor(emp.id)?.attendance?.late_approved_at"
                                class="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700"
                                title="متأخر"
                            >
                                <span class="h-1.5 w-1.5 rounded-full bg-amber-500" />
                                متأخر {{ analyticsFor(emp.id).attendance.late_minutes ? `${analyticsFor(emp.id).attendance.late_minutes} د` : '' }}
                            </span>
                            <button
                                v-if="analyticsFor(emp.id)?.attendance?.is_late && !analyticsFor(emp.id)?.attendance?.late_approved_at && canAssignAdmin"
                                type="button"
                                class="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-700 hover:bg-blue-200"
                                title="اعتماد التأخير"
                                @click="approveLate(analyticsFor(emp.id).attendance.attendance_id)"
                            >
                                اعتماد
                            </button>
                            <span
                                v-if="analyticsFor(emp.id)?.attendance?.late_approved_at"
                                class="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-700"
                                title="تم اعتماد التأخير"
                            >
                                تم اعتماد التأخير
                            </span>
                            <span
                    ...
                    <div class="space-y-3 px-4 pb-4">
                        ...
                        <!-- Today's late reason (if late and reason provided) -->
                        <div
                            v-if="analyticsFor(emp.id)?.attendance?.is_late && analyticsFor(emp.id)?.attendance?.late_reason"
                            class="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-2 text-[11px] leading-relaxed text-amber-900"
                        >
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-600" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" /></svg>
                            <div class="min-w-0 flex-1">
                                <div class="text-[9px] font-bold uppercase tracking-wider text-amber-700">سبب التأخير</div>
                                <div class="mt-0.5 whitespace-pre-line">{{ analyticsFor(emp.id).attendance.late_reason }}</div>
                            </div>
                        </div>
        ...
        <Modal :show="editModalOpen" @close="closeEditModal">
            <div class="glass-modal employee-light-modal p-4 sm:p-6">
                ...
                <form class="mt-4 space-y-4" @submit.prevent="submitEdit">
                    ...
                    <div>
                        <InputLabel for="e_late_threshold" value="وقت بداية التأخير" />
                        <TextInput id="e_late_threshold" v-model="editForm.late_threshold_time" type="time" class="mt-1 block w-full" />
                        <p class="mt-0.5 text-[11px] text-gray-500">بعد هذه الساعة يعتبر الحضور متأخراً.</p>
                    </div>

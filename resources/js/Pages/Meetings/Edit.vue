...
const form = useForm({
    ...
    goods_meta_lead_id: props.meeting.goods_meta_lead_id ?? null,
...
function submit() { form.patch(route('meetings.update', props.meeting.id)); }

function destroyMeeting() {
    if (!confirm('حذف هذا الاجتماع؟ لا يمكن التراجع.')) return;
...
function selectLead(lead) {
    ...
    form.invitee_email = '';
    if (!form.title || form.title === `اجتماع مع ${props.meeting.invitee_name}`) {
        form.title = `اجتماع مع ${lead.full_name}`;
    }
        ...
        <div class="mx-auto max-w-2xl space-y-4">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white px-4 py-5 shadow-sm sm:p-6">
                ...
                <div class="mt-8 border-t border-gray-100 pt-6">
                    <p class="text-sm text-gray-500">حذف الاجتماع من السجل الداخلي.</p>
                    <DangerButton type="button" class="mt-3 min-h-11" @click="destroyMeeting">
                        حذف الاجتماع
                    </DangerButton>
                </div>
            </div>

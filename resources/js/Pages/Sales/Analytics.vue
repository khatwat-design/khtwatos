...
const props = defineProps({
    ...
    meta_leads: { type: Object, default: () => ({ reps: [], team_totals: {}, by_campaign: {}, funnel: {}, campaign_performance: [], daily_trend: [], response_time_distribution: [] }) },
});
...
function gradeClass(grade) {
    switch (grade) {
        ...
        default: return 'bg-rose-100 text-rose-800 ring-1 ring-rose-300';
    }
}
...
function signalToneClass(tone) {
    switch (tone) {
        case 'emerald': return 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200';
        case 'sky': return 'bg-sky-50 text-sky-800 ring-1 ring-sky-200';
        case 'rose': return 'bg-rose-50 text-rose-800 ring-1 ring-rose-200';
        case 'orange': return 'bg-orange-50 text-orange-800 ring-1 ring-orange-200';
        case 'amber': return 'bg-amber-50 text-amber-800 ring-1 ring-amber-200';
        default: return 'bg-slate-100 text-slate-700 ring-1 ring-slate-200';
    }
}

function funnelColor(color) {
    const map = {
        slate: 'bg-slate-500',
        sky: 'bg-sky-500',
        amber: 'bg-amber-500',
...
function funnelTextColor(color) {
    const map = {
...
const totalPipeline = computed(() => Object.values(props.pipeline || {}).reduce((a, b) => a + Number(b || 0), 0));
const totalChannels = computed(() => Object.values(props.channel_mix || {}).reduce((a, b) => a + Number(b || 0), 0));
...
const sortedReps = computed(() => {
    ...
    arr.sort((a, b) => {
        switch (sortBy.value) {
            case 'conversion': return (b.quality?.conversion_rate || 0) - (a.quality?.conversion_rate || 0);
            case 'volume': return (b.volume?.messages_out || 0) - (a.volume?.messages_out || 0);
            case 'response': return (a.quality?.first_response_avg_seconds || Infinity) - (b.quality?.first_response_avg_seconds || Infinity);
            case 'stalled': return (b.quality?.stalled_conversations || 0) - (a.quality?.stalled_conversations || 0);
            case 'score': default: return (b.score || 0) - (a.score || 0);
        }
    });
    return arr;
});

const funnel = computed(() => props.meta_leads?.funnel || {});
const campaignPerf = computed(() => props.meta_leads?.campaign_performance || []);
const dailyTrend = computed(() => props.meta_leads?.daily_trend || []);
const responseDist = computed(() => props.meta_leads?.response_time_distribution || []);

const trendMaxTotal = computed(() => Math.max(1, ...dailyTrend.value.map(d => d.total)));
const trendMaxWon = computed(() => Math.max(1, ...dailyTrend.value.map(d => d.won)));
const respTotal = computed(() => responseDist.value.reduce((s, r) => s + r.count, 0));
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto w-full min-w-0 max-w-6xl space-y-4 px-3 pb-8 sm:px-4">
            ...
            <div class="ui-card flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-600">
                        تتبّع الكمية والجودة لكل مندوب: عدد العملاء، سرعة الرد، نسبة التحويل، توزيع الحالات.
                    </p>
                </div>
            ...
            <!-- ═══════════ META LEADS: CONVERSION FUNNEL ═══════════ -->
            <div v-if="funnel.steps" class="ui-card p-4">
                <div class="mb-3 flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-bold text-slate-900">قمع التحويل — ليدز الإعلانات</h2>
                        <p class="text-[11px] text-slate-500">من استلام الليدز إلى إتمام الصفقة</p>
                    </div>
                    <Link
                        :href="route('sales.lead-assignment')"
                        class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 hover:bg-slate-50"
                    >
                        إدارة التوزيع
                    </Link>
                </div>
                <div class="flex items-end gap-2 overflow-x-auto pb-2">
                    <div
                        v-for="(step, idx) in funnel.steps"
                        :key="`fs-${idx}`"
                        class="flex min-w-[140px] flex-1 flex-col items-center gap-1"
                    >
                        <div
                            class="w-full rounded-xl border p-3 text-center transition-all"
                            :class="funnelBgColor(step.color)"
                        >
                            <div class="text-2xl font-black" :class="funnelTextColor(step.color)">{{ step.value }}</div>
                            <div class="mt-1 text-[10px] font-semibold" :class="funnelTextColor(step.color)">{{ step.label }}</div>
                            <div v-if="idx > 0 && funnel.steps[0].value > 0" class="mt-1 text-[10px] text-slate-500">
                                {{ Math.round((step.value / funnel.steps[0].value) * 100) }}%
                            </div>
                        </div>
                        <div v-if="idx < funnel.steps.length - 1" class="hidden text-lg text-slate-300 sm:block">←</div>
                    </div>
                </div>
                <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                    <div class="rounded-lg bg-slate-50 px-3 py-2 text-center">
                        <div class="text-[10px] text-slate-500">نسبة التواصل</div>
                        <div class="text-sm font-bold text-sky-700">{{ funnel.contact_rate }}%</div>
                    </div>
                    <div class="rounded-lg bg-slate-50 px-3 py-2 text-center">
                        <div class="text-[10px] text-slate-500">نشطة حالياً</div>
                        <div class="text-sm font-bold text-amber-700">{{ funnel.active }}</div>
                    </div>
                    <div class="rounded-lg bg-slate-50 px-3 py-2 text-center">
                        <div class="text-[10px] text-slate-500">ساخن/دافئ</div>
                        <div class="text-sm font-bold text-orange-700">{{ funnel.hot + funnel.warm }}</div>
                    </div>
                    <div class="rounded-lg bg-slate-50 px-3 py-2 text-center">
                        <div class="text-[10px] text-slate-500">نسبة الاتفاق</div>
                        <div class="text-sm font-bold text-emerald-700">{{ funnel.won_rate }}%</div>
                    </div>
                </div>
            </div>

            <!-- ═══════════ META LEADS: DAILY TREND ═══════════ -->
            <div v-if="dailyTrend.length" class="ui-card p-4">
                <div class="mb-3">
                    <h2 class="text-sm font-bold text-slate-900">الاتجاه اليومي</h2>
                    <p class="text-[11px] text-slate-500">عدد الليدز والتحويلات اليومية</p>
                </div>
                <div class="flex items-end gap-px overflow-x-auto" style="height: 120px;">
                    <div
                        v-for="day in dailyTrend"
                        :key="`dt-${day.date}`"
                        class="group relative flex min-w-[28px] flex-1 flex-col items-center justify-end"
                    >
                        <div class="absolute -top-6 hidden whitespace-nowrap rounded bg-slate-800 px-1.5 py-0.5 text-[9px] text-white group-hover:block">
                            {{ fmtDate(day.date) }}: {{ day.total }} ليدز
                        </div>
                        <div class="w-full rounded-t bg-sky-200 transition-all" :style="{ height: ((day.total / trendMaxTotal) * 100) + '%' }">
                            <div
                                v-if="day.won > 0"
                                class="w-full rounded-t bg-emerald-500"
                                :style="{ height: ((day.won / day.total) * 100) + '%' }"
                            />
                        </div>
                    </div>
                </div>
                <div class="mt-2 flex items-center gap-4 text-[10px] text-slate-500">
                    <span class="flex items-center gap-1"><span class="inline-block h-2 w-2 rounded bg-sky-300" /> ليدز</span>
                    <span class="flex items-center gap-1"><span class="inline-block h-2 w-2 rounded bg-emerald-500" /> تم الاتفاق</span>
                </div>
            </div>

            <!-- ═══════════ META LEADS: CAMPAIGN PERFORMANCE ═══════════ -->
            <div v-if="campaignPerf.length" class="ui-card p-4">
                <div class="mb-3">
                    <h2 class="text-sm font-bold text-slate-900">أداء الحملات</h2>
                    <p class="text-[11px] text-slate-500">مقارنة أداء كل حملة إعلانية</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-[11px]">
                        <thead>
                            <tr class="border-b border-slate-200 text-start text-[10px] font-semibold text-slate-500">
                                <th class="px-2 py-1.5 text-start">الحملة</th>
                                <th class="px-2 py-1.5 text-center">الليدز</th>
                                <th class="px-2 py-1.5 text-center">تم التواصل</th>
                                <th class="px-2 py-1.5 text-center">الساخن</th>
                                <th class="px-2 py-1.5 text-center">متابعة</th>
                                <th class="px-2 py-1.5 text-center">تم الاتفاق</th>
                                <th class="px-2 py-1.5 text-center">نسبة التواصل</th>
                                <th class="px-2 py-1.5 text-center">نسبة التحويل</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="c in campaignPerf" :key="`cp-${c.name}`" class="border-b border-slate-100 hover:bg-slate-50/60">
                                <td class="max-w-[200px] truncate px-2 py-1.5 font-semibold text-slate-800">{{ c.name }}</td>
                                <td class="px-2 py-1.5 text-center font-bold text-slate-900">{{ c.total }}</td>
                                <td class="px-2 py-1.5 text-center text-sky-700">{{ c.contacted }}</td>
                                <td class="px-2 py-1.5 text-center text-orange-700">{{ c.hot }}</td>
                                <td class="px-2 py-1.5 text-center text-slate-700">{{ c.following }}</td>
                                <td class="px-2 py-1.5 text-center font-bold text-emerald-700">{{ c.won }}</td>
                                <td class="px-2 py-1.5 text-center">
                                    <span class="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold text-sky-800">{{ c.contact_rate }}%</span>
                                </td>
                                <td class="px-2 py-1.5 text-center">
                                    <span
                                        class="rounded-full px-2 py-0.5 text-[10px] font-semibold"
                                        :class="c.conversion_rate >= 30 ? 'bg-emerald-100 text-emerald-800' : c.conversion_rate >= 15 ? 'bg-amber-100 text-amber-800' : 'bg-rose-100 text-rose-800'"
                                    >
                                        {{ c.conversion_rate }}%
                                    </span>
                                </td>
            ...
            <!-- ═══════════ META LEADS: RESPONSE TIME ═══════════ -->
            <div v-if="respTotal > 0" class="ui-card p-4">
                <div class="mb-3">
                    <h2 class="text-sm font-bold text-slate-900">توزيع أوقات التواصل الأول</h2>
                    <p class="text-[11px] text-slate-500">كم من الوقت أخذ التواصل الأول مع العميل</p>
                </div>
                <div class="grid grid-cols-4 gap-2">
                    <div
                        v-for="r in responseDist"
                        :key="`rd-${r.label}`"
                        class="rounded-xl border p-3 text-center"
                        :class="r.color === 'emerald' ? 'border-emerald-200 bg-emerald-50' : r.color === 'sky' ? 'border-sky-200 bg-sky-50' : r.color === 'amber' ? 'border-amber-200 bg-amber-50' : 'border-rose-200 bg-rose-50'"
                    >
                        <div class="text-lg font-black" :class="r.color === 'emerald' ? 'text-emerald-800' : r.color === 'sky' ? 'text-sky-800' : r.color === 'amber' ? 'text-amber-800' : 'text-rose-800'">
                            {{ r.count }}
                        </div>
                        <div class="text-[10px] font-semibold" :class="r.color === 'emerald' ? 'text-emerald-700' : r.color === 'sky' ? 'text-sky-700' : r.color === 'amber' ? 'text-amber-700' : 'text-rose-700'">
                            {{ r.label }}
                        </div>
                        <div class="mt-0.5 text-[9px] text-slate-500">{{ respTotal > 0 ? Math.round((r.count / respTotal) * 100) : 0 }}%</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Meta leads per-rep summary -->
            <div v-if="(meta_leads?.reps || []).length" class="ui-card space-y-4 p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-bold text-slate-900">العملاء المحتملون (إعلانات) — أداء المُعيَّنين</h2>
                        <p class="mt-0.5 text-xs text-slate-600">
                            أداء الفترة {{ meta_leads.range_days }} يوم
                        </p>
                    </div>
                    <Link
                        :href="route('sales.lead-assignment')"
                ...
                <div class="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-slate-100 bg-slate-100 sm:grid-cols-4">
                    ...
                    <div class="bg-white p-3">
                        <p class="text-[11px] text-slate-500">متابعة</p>
                        <p class="text-xl font-bold text-slate-800">{{ meta_leads.team_totals?.following ?? 0 }}</p>
                    </div>
                    <div class="bg-white p-3">
                        <p class="text-[11px] text-slate-500">تم الاتفاق</p>
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    ...
                    >
                        ...
                        <dl class="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                            ...
                            <div>
                                <dt class="text-slate-500">ساخن/دافئ</dt>
                                <dd class="font-bold text-orange-700">{{ (rep.hot || 0) + (rep.warm || 0) }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="font-bold text-sky-700">{{ rep.following }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-500">لم يُتواصل</dt>
                                <dd class="font-bold text-rose-700">{{ rep.not_contacted }}</dd>
                            </div>
                            <div>

...
const props = defineProps({
    ...
    meetings: Object,
    employees: Object,
    sales_dashboard: { type: Object, default: () => ({}) },
});
    ...
    if (props.staff?.is_meta_leads_rep) {
        cards.unshift(
            ...
            },
            {
                ...
                title: 'عملاء محتملون اليوم',
                value: Number(c.meta_leads_today || 0),
                sub: 'عملاء جدد اليوم مسندين إليك',
                route: 'goods.index',
...
const sdFunnel = computed(() => {
    const d = sd.value;
    if (!d.total) return [];
    return [
        { label: 'الكل', value: d.total, color: 'bg-slate-500', textColor: 'text-slate-800' },
        { label: 'ساخن/دافئ', value: (d.hot || 0) + (d.warm || 0), color: 'bg-orange-500', textColor: 'text-orange-800' },
        { label: 'متابعة', value: d.following || 0, color: 'bg-sky-500', textColor: 'text-sky-800' },
        { label: 'محتمل', value: d.potential || 0, color: 'bg-amber-500', textColor: 'text-amber-800' },
        { label: 'تم الاتفاق', value: d.won || 0, color: 'bg-emerald-500', textColor: 'text-emerald-800' },
    ];
});

const sdMaxStatus = computed(() => Math.max(1, ...(sd.value.status_breakdown || []).map(s => s.count)));
...
const adminAttentionItems = computed(() => {
    ...
    if (items.length === 0) {
        items.push({
            ...
            detail: 'لا مهام متأخرة في لقطة البيانات الحالية ضمن العتبات المعروضة.',
            route: null,
            action_label: null,
        });
    }
...
const sd = computed(() => props.sales_dashboard || {});
const isSalesRep = computed(() => Boolean(sd.value.total !== undefined));

const statusLabels = {
    kull: 'الكل', sakhin: 'الساخن', dafae: 'دافئ', barid: 'بارد',
    moutafaq: 'تم الاتفاق', lam_yattasel: 'لم يُتواصل', moshtarek: 'مشترك',
    istihqaq: 'استحقاق', istihdaf: 'استهداف', matabaa: 'متابعة',
    mohtamal: 'محتمل', khutwat: 'خطوات', khat: 'خط',
};
const statusColors = {
    kull: 'bg-slate-400', sakhin: 'bg-orange-500', dafae: 'bg-amber-400',
    barid: 'bg-sky-400', moutafaq: 'bg-emerald-500', lam_yattasel: 'bg-rose-400',
    moshtarek: 'bg-indigo-500', istihqaq: 'bg-amber-500', istihdaf: 'bg-teal-500',
    matabaa: 'bg-sky-500', mohtamal: 'bg-amber-400', khutwat: 'bg-emerald-400', khat: 'bg-slate-400',
};
const statusBgLight = {
    kull: 'bg-slate-50', sakhin: 'bg-orange-50', dafae: 'bg-amber-50',
    barid: 'bg-sky-50', moutafaq: 'bg-emerald-50', lam_yattasel: 'bg-rose-50',
    moshtarek: 'bg-indigo-50', istihqaq: 'bg-amber-50', istihdaf: 'bg-teal-50',
    matabaa: 'bg-sky-50', mohtamal: 'bg-amber-50', khutwat: 'bg-emerald-50', khat: 'bg-slate-50',
};
...
function fmtCallCountdown(iso) {
    if (!iso) return '';
    const diff = new Date(iso).getTime() - Date.now();
    if (diff < 0) return 'فاتت';
    const mins = Math.floor(diff / 60000);
    if (mins < 60) return `خلال ${mins} د`;
    const hrs = Math.floor(mins / 60);
    return `خلال ${hrs}س ${mins % 60}د`;
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        >
            ...
            <!-- لوحة المبيعات: تظهر فقط لمندوب المبيعات -->
            <template v-if="isSalesRep">
                <!-- KPI Cards المبيعات -->
                <section class="ops-surface overflow-hidden p-0 shadow-sm">
                    <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                        <p class="ops-kicker">ملخص عملك</p>
                        <p class="ops-section-lead mt-0.5 text-ops-meta">نظرة سريعة على ليدزك وحالة التحويل</p>
                    </div>
                    <div class="grid grid-cols-2 divide-x divide-slate-100 divide-y divide-slate-100 sm:grid-cols-3 lg:grid-cols-6 lg:divide-y-0">
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">إجمالي الليدز</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-slate-900 sm:text-[1.65rem]">{{ sd.total }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">{{ sd.today }} ليد جديد اليوم</p>
                        </article>
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">ساخن / دافئ</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-orange-600 sm:text-[1.65rem]">{{ (sd.hot || 0) + (sd.warm || 0) }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">الساخن {{ sd.hot || 0 }} · الدافئ {{ sd.warm || 0 }}</p>
                        </article>
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">متابعة</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-sky-600 sm:text-[1.65rem]">{{ sd.following || 0 }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">قيد المتابعة النشطة</p>
                        </article>
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">تم الاتفاق</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-emerald-600 sm:text-[1.65rem]">{{ sd.won || 0 }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">نسبة التحويل {{ sd.conversion_rate || 0 }}%</p>
                        </article>
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">لم يُتواصل</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-rose-500 sm:text-[1.65rem]">{{ sd.not_contacted || 0 }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">يحتاج تواصل أولي</p>
                        </article>
                        <article class="min-h-[6rem] bg-white px-4 py-3 sm:px-4 sm:py-4">
                            <p class="text-ops-label font-semibold text-slate-500">اتصالات قادمة</p>
                            <p class="mt-1.5 text-2xl font-black tabular-nums text-indigo-600 sm:text-[1.65rem]">{{ sd.upcoming_calls_count || 0 }}</p>
                            <p class="mt-1 text-ops-meta leading-snug text-slate-500">مواعيد محددة مسبقاً</p>
                        </article>
                    </div>
                </section>
                ...
                <section class="grid gap-4 lg:grid-cols-2">
                    <!-- Funnel -->
                    <div class="ops-surface overflow-hidden p-0 shadow-sm">
                        <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                            <p class="ops-kicker">قمع التحويل</p>
                            <p class="ops-section-lead mt-0.5 text-ops-meta">من الكل إلى Agreement</p>
                        </div>
                        <div class="px-4 py-4 sm:px-5">
                            <div class="space-y-2.5">
                                <div v-for="(step, fi) in sdFunnel" :key="`funnel-${fi}`" class="flex items-center gap-3">
                                    <span class="w-24 shrink-0 text-end text-[12px] font-semibold text-slate-700">{{ step.label }}</span>
                                    <div class="flex-1 overflow-hidden rounded-full bg-slate-100">
                                        <div
                                            class="h-7 rounded-full transition-[width] duration-500 ease-out flex items-center justify-center"
                                            :class="step.color"
                                            :style="{ width: `${sd.total ? Math.max(8, (step.value / sd.total) * 100) : 0}%` }"
                                        >
                                            <span class="text-[11px] font-bold text-white tabular-nums">{{ step.value }}</span>
                                        </div>
                                    </div>
                                    <span class="w-10 text-left text-[11px] font-bold tabular-nums text-slate-500">
                                        {{ sd.total ? Math.round((step.value / sd.total) * 100) : 0 }}%
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="ops-surface overflow-hidden p-0 shadow-sm">
                        <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                            <p class="ops-kicker">التوزيع حسب الحالة</p>
                            <p class="ops-section-lead mt-0.5 text-ops-meta">عدد الليدز في كل حالة عمل</p>
                        </div>
                        <div class="px-4 py-4 sm:px-5">
                            <div v-if="sd.status_breakdown?.length" class="space-y-2">
                                <div v-for="s in sd.status_breakdown" :key="`sb-${s.status}`" class="flex items-center gap-3">
                                    <span
                                        class="inline-block h-2.5 w-2.5 shrink-0 rounded-full"
                                        :class="statusColors[s.status] || 'bg-slate-300'"
                                    />
                                    <span class="w-20 shrink-0 text-end text-[12px] font-semibold text-slate-700">{{ s.label }}</span>
                                    <div class="flex-1 overflow-hidden rounded-full bg-slate-100">
                                        <div
                                            class="h-5 rounded-full transition-[width] duration-300"
                                            :class="statusColors[s.status] || 'bg-slate-300'"
                                            :style="{ width: `${(s.count / sdMaxStatus) * 100}%` }"
                                        />
                                    </div>
                                    <span class="w-8 text-left text-[12px] font-bold tabular-nums text-slate-800">{{ s.count }}</span>
                                </div>
                            </div>
                            <p v-else class="py-6 text-center text-sm text-slate-500">لا توجد بيانات حالات بعد.</p>
                        </div>
                    </div>
                </section>

                <!-- Upcoming Calls -->
                <section v-if="sd.upcoming_calls?.length" class="ops-surface overflow-hidden p-0 shadow-sm">
                    <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                        <p class="ops-kicker">اتصالات قادمة</p>
                        <p class="ops-section-lead mt-0.5 text-ops-meta">أقرب مواعيد الاتصال المحددة لليدز</p>
                    </div>
                    <ul class="divide-y divide-slate-100" role="list">
                        <li v-for="call in sd.upcoming_calls" :key="`call-${call.id}`">
                            <Link
                                :href="route('goods.meta_leads.show', call.id)"
                                class="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-brand-50/30 sm:px-5 sm:py-3.5"
                            >
                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[13px] font-black text-brand-700">
                                    {{ (call.name || '—').charAt(0) }}
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-[13px] font-semibold text-slate-900">{{ call.name }}</p>
                                    <p class="text-[11px] text-slate-500">{{ call.phone }} · {{ call.campaign_name || '—' }}</p>
                                </div>
                                <div class="shrink-0 text-left">
                                    <p class="text-[12px] font-semibold tabular-nums text-indigo-700">{{ fmtDateTimeShort(call.next_call_at) }}</p>
                                    <p class="text-[10px] font-medium text-indigo-500">{{ fmtCallCountdown(call.next_call_at) }}</p>
                                </div>
                            </Link>
                        </li>
                    </ul>
                ...
                <!-- Recent Activity -->
                <section v-if="sd.recent_activity?.length" class="ops-surface overflow-hidden p-0 shadow-sm">
                    <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                        <p class="ops-kicker">آخر التغييرات</p>
                        <p class="ops-section-lead mt-0.5 text-ops-meta">آخر 5 تغييرات حالة على ليدزك</p>
                    </div>
                    <ul class="divide-y divide-slate-100" role="list">
                        <li v-for="act in sd.recent_activity" :key="`act-${act.id}`">
                            <Link
                                :href="route('goods.meta_leads.show', act.lead_id)"
                                class="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-slate-50 sm:px-5 sm:py-3.5"
                            >
                                <span class="mt-1 h-2 w-2 shrink-0 rounded-full bg-slate-400" />
                                <div class="min-w-0 flex-1">
                                    <p class="text-[13px] font-semibold text-slate-900">{{ act.lead_name }}</p>
                                    <p class="text-[11px] text-slate-500">
                                        <span class="font-medium text-slate-600">{{ statusLabels[act.from_status] || act.from_status }}</span>
                                        →
                                        <span class="font-semibold text-slate-800">{{ statusLabels[act.to_status] || act.to_status }}</span>
                                    </p>
                                    <p v-if="act.note" class="mt-0.5 truncate text-[10px] text-slate-400">{{ act.note }}</p>
                                </div>
                                <span class="shrink-0 text-[10px] tabular-nums text-slate-400">{{ fmtDateTimeShort(act.created_at) }}</span>
                            </Link>
                        </li>
                    </ul>
                </section>
            ...
            <!-- KPIs العامة: تظهر فقط لغير مندوب المبيعات -->
            <template v-else>
                <section class="ops-surface overflow-hidden p-0 shadow-sm">
                    <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                        ...
                        <li v-for="m in staff.upcoming_meetings" :key="`um-${m.id}`">
                            ...
                            >
                                ...
                                <p class="mt-1 flex flex-wrap gap-x-2 gap-y-0.5 text-[11px] text-slate-500">
                                    ...
                                    <span v-if="m.meta_lead">
                                        <span class="font-medium text-brand-600">{{ m.meta_lead.name }}</span>
                                        <span v-if="m.meta_lead.phone" class="text-slate-400" dir="ltr">· {{ m.meta_lead.phone }}</span>
                                    </span>
                                    <span v-else-if="m.client_name">· {{ m.client_name }}</span>
                                    <span
                                        v-if="m.is_host"

...
const mood = ref('good');
const note = ref('');
const showLateReason = ref(false);
...
async function submit() {
    ...
    try {
        ...
        if (showLateReason.value) {
            payload.late_reason = lateReason.value;
        }
        ...
        if (res.data?.is_late && !showLateReason.value) {
            showLateReason.value = true;
    ...
    >
        ...
        >
            ...
            <div class="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                ...
                <!-- Late reason -->
                <div v-if="showLateReason" class="rounded-xl border border-amber-200 bg-amber-50/70 px-4 py-3">
                    <div class="mb-1 flex items-center justify-between">
                        <div class="text-sm font-semibold text-amber-900">سبب التأخير</div>
                        <span class="text-[10px] text-amber-600">{{ lateReason.length }}/480</span>
                    </div>
                    <textarea
                        v-model="lateReason"
                        rows="2"
                        maxlength="480"
                        placeholder="مثال: ازدحام مروري، ظرف طارئ..."
                        class="w-full rounded-xl border border-amber-300 bg-white px-3 py-2 text-sm text-amber-900 placeholder-amber-400 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-400"
                    />
                    <p class="mt-1 text-[11px] text-amber-700">
                        سيتم تسجيل سبب التأخير للإدارة.
                    </p>
                </div>
            </div>
            ...
            <div class="shrink-0 border-t border-slate-200 bg-slate-50/80 px-5 py-3">
                <div class="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-[11px] text-slate-500">
                        {{ showLateReason ? 'سيتم تسجيل سبب التأخير مع وقت الحضور.' : 'سيبدأ احتساب وقت دوامك تلقائياً فور تأكيد الحضور.' }}
                    </p>
                    <button
                        type="button"
                        :disabled="submitting"
                    ...
                    >
                        ...
                        <span>{{ submitting ? 'جاري الحفظ...' : showLateReason ? 'إرسال سبب التأخير' : 'تأكيد الحضور وبدء الدوام' }}</span>
                    </button>
                </div>
            </div>
        </div>

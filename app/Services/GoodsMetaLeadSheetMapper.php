...
class GoodsMetaLeadSheetMapper
{
    ...
    public function mapRow(array $row): array
    {
        ...
        $monthly = $this->stringValue($normalized, [
            ...
            'كمعددطلباتكاليوميهحاليا؟',
            'monthly_orders_answer',
        ]);
        $goal = $this->stringValue($normalized, [
            'شنوالمشكلةالليتواجهكاوشنوهدفكمنالعملويانه',
            'ماهيأبرزالمشاكلالليواجهابالمشروعحالياً؟',
            'goal_answer',
        ]);

        $projectType = $this->stringValue($normalized, ['نوعمشروعك؟']);
        $products = $this->stringValue($normalized, ['اذاعندكمنتجاتاملئهذاالحقل؟']);
        $budget = $this->stringValue($normalized, ['تقدرتصرفمن50$إلى400$فياليوم؟', 'ميزانيةالترويجاليومية؟']);
        $brand = $this->stringValue($normalized, ['اذاعندكبراندشاركنارابطالموقعأوالانستجرام؟']);
        $city = $this->stringValue($normalized, ['city', 'المدينة']);
        ...
        return [
            ...
            'full_name' => $this->stringValue($normalized, ['full_name', 'name', 'الأسم', 'الاسم']),
            'phone' => $this->cleanPhone($this->stringValue($normalized, ['phone_number', 'phone', 'رقمالهاتف', 'الهاتف'])),
            ...
            'first_contact_date' => $this->parseSheetDate($normalized['تاريخالاتصالالاول'] ?? $normalized['last_contact_date'] ?? null),
            ...
            'next_call_at' => $this->parseDateTime($normalized['الموعدالقادم'] ?? $normalized['الأتصالالقادم'] ?? null)
                ?? $this->parseSheetDate($normalized['الموعدالقادم'] ?? $normalized['الأتصالالقادم'] ?? null),
    ...
    private function knownKeys(): array
    {
        return [
            ...
            'اذاعندكبراندشاركنارابطالموقعأوالانستجرام؟', 'city', 'المدينة',
            'الملاحظات', 'احتماليةالعميل', 'فيدباكالمكالمة', 'الفيديوالليجهمنه', 'الموعدالقادم',
            ...
            'الأسم', 'رقمالهاتف', 'آخرإتصال', 'الأتصالالقادم',
            'كمعددطلباتكاليوميهحاليا؟', 'ماهيأبرزالمشاكلالليواجهابالمشروعحالياً؟',
            'ميزانيةالترويجاليومية؟', 'ماهوالهدفالمطلوبمنالتواصلمعنا؟',
            '_row_number',
        ];
    }

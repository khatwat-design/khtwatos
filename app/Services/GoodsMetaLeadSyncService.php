...
class GoodsMetaLeadSyncService
{
    public function __construct(
        ...
        private readonly LeadAutoOutreachService $outreach,
    ) {}
    ...
    public function upsertFromSheetRow(array $row, ?int $actorUserId = null): ?GoodsMetaLead
    {
        ...
        return DB::transaction(function () use ($mapped, $actorUserId) {
            ...
            if (! $existing) {
                ...
                // AI Bot: إرسال رسالة ترحيب تلقائية لليد الجديد
                try {
                    $this->outreach->sendGreetingToNewLead($lead);
                } catch (\Throwable) {
                    // Silent fail — greeting is best-effort
                }

...
class EmployeePresenceService
{
     ...
     *
     * @param  array{status?: string, mood?: ?string, plan_for_today?: ?string, note?: ?string, late_reason?: ?string}  $payload
     */
    public function checkIn(User $user, array $payload): EmployeeAttendance
    {
        ...
        if ($attendance->checked_in_at === null) {
            ...
            $this->applyLateFlag($user, $attendance);
        }

        $attendance->late_reason = isset($payload['late_reason'])
            ? mb_substr((string) $payload['late_reason'], 0, 480)
    ...
    private function applyLateFlag(User $user, EmployeeAttendance $attendance): void
    {
        if ($attendance->checked_in_at === null) {
            return;
        }

        $threshold = $user->late_threshold_time ?? '11:00';
        $checkInTime = $attendance->checked_in_at->format('H:i');

        if ($checkInTime > $threshold) {
            $attendance->is_late = true;
            $checkInCarbon = $attendance->checked_in_at;
            $thresholdCarbon = $checkInCarbon->copy()->setTimeFromTimeString($threshold);
            $attendance->late_minutes = (int) max(0, $thresholdCarbon->diffInMinutes($checkInCarbon));
        }
    }

...
class EmployeeController extends Controller
{
        ...
        return Inertia::render('Employees/Index', [
            ...
            'employees' => $employees->map(function (User $user) {
                ...
                return [
                    ...
                    'late_threshold_time' => $user->late_threshold_time ? substr((string) $user->late_threshold_time, 0, 5) : '11:00',
    ...
    public function update(Request $request, User $employee): RedirectResponse
    {
        ...
        $employee->fill([
            ...
            'availability_end_time' => $data['availability_end_time'],
            'availability_schedule' => $data['availability_schedule'],
            'late_threshold_time' => $data['late_threshold_time'] ?? '11:00',
        ]);
    ...
    private function validatedEmployee(Request $request, bool $isCreate, ?int $userId = null): array
    {
        ...
        $data = $request->validate([
            ...
            'late_threshold_time' => ['nullable', 'date_format:H:i'],
        ]);
    ...
    public function approveLate(Request $request, EmployeeAttendance $attendance): RedirectResponse
    {
        $user = $request->user();
        if (! $user || (! $user->isAdmin() && ! $user->isHrManager())) {
            abort(403);
        }

        $attendance->update([
            'late_approved_at' => now(),
            'late_approved_by_id' => $user->id,
        ]);

        return redirect()->route('employees.index')->with('success', 'تم اعتماد التأخير.');
    }

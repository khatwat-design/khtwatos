...
class OutsideWebhookController extends Controller
{
    public function __construct(
        ...
        private readonly ConversationFlowService $botFlow,
    ) {}
    ...
    private function storeInboundMessengerMessage(array $messaging, ?int $clientId, string $pageId): void
    {
        ...
        // AI Bot: رد تلقائي على الرسائل الواردة
        try {
            $this->botFlow->handleInboundMessage($conversation->fresh(), $preview);
        } catch (\Throwable $e) {
            Log::warning('bot.reply.failed', ['conversation_id' => $conversation->id, 'error' => $e->getMessage()]);
        }
    }
    ...
    private function storeInboundInstagramMessage(array $messaging, ?int $clientId, string $igBusinessAccountId): void
    {
        ...
        // AI Bot: رد تلقائي على رسائل إنستغرام
        try {
            $this->botFlow->handleInboundMessage($conversation->fresh(), $preview);
        } catch (\Throwable $e) {
            Log::warning('bot.reply.failed.instagram', ['conversation_id' => $conversation->id, 'error' => $e->getMessage()]);
        }
    }
    ...
    private function storeInboundMessage(array $incoming, array $profileNamesByPhone): void
    {
        ...
        // AI Bot: رد تلقائي على رسائل واتساب
        try {
            $this->botFlow->handleInboundMessage($conversation->fresh(), $preview);
        } catch (\Throwable $e) {
            Log::warning('bot.reply.failed.whatsapp', ['conversation_id' => $conversation->id, 'error' => $e->getMessage()]);
        }
    }

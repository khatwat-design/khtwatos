...
class HomeController extends Controller
{
    ...
    private function renderStaffDashboard(User $user): Response
    {
        ...
        $upcomingMeetings = Meeting::query()
            ->with(['client:id,name', 'goodsMetaLead:id,full_name,phone,workflow_status,campaign_name'])
            ->where('status', 'scheduled')
            ->where('start_at', '>=', $now)
            ->where(function ($q) use ($user) {
                $q->where('user_id', $user->id)
        ...
        $salesDashboard = $this->buildSalesDashboard($user);
    ...
    private function buildStaffPriorityFollowups(
        ...
        if ($items === []) {
            $items[] = [
                ...
                'detail' => 'لا مهام متأخرة ولا اجتماعات خلال يومين. يمكنك متابعة عملك من الشريط الجانبي.',
                'route' => null,
                'action_label' => null,
            ];
        }
    ...
    private function buildSalesDashboard(User $user): array
    {
        $isAssignee = app(GoodsMetaLeadAssignmentService::class)->isAssignee((int) $user->id);
        if (! $isAssignee) {
            return [];
        }

        $myLeads = GoodsMetaLead::query()->where('owner_user_id', $user->id);
        $today = now()->startOfDay();

        $byStatus = (clone $myLeads)->get(['workflow_status'])
            ->groupBy('workflow_status')
            ->map->count()
            ->all();

        $total = (clone $myLeads)->count();
        ...
        $todayLeads = (clone $myLeads)->where(function ($q) use ($today) {
            $q->whereDate('lead_created_at', $today)
                ->orWhereDate('created_at', $today);
        })->count();

        $upcomingCalls = (clone $myLeads)
            ->whereNotNull('next_call_at')
            ->where('next_call_at', '>=', now())
            ->orderBy('next_call_at')
            ->limit(5)
            ->get([
                'id', 'full_name', 'phone', 'workflow_status', 'next_call_at', 'campaign_name',
            ])
            ->map(fn (GoodsMetaLead $l) => [
                'id' => $l->id,
                'name' => $l->full_name ?: '—',
                'phone' => $l->phone ?: '—',
                'status' => $l->workflow_status,
                'next_call_at' => $l->next_call_at?->toIso8601String(),
                'campaign_name' => $l->campaign_name,
            ])
            ->values();

        $upcomingCallsCount = (clone $myLeads)
            ->whereNotNull('next_call_at')
            ->where('next_call_at', '>=', now())
            ->count();

        $recentActivity = GoodsMetaLeadStatusHistory::query()
            ->whereHas('lead', fn ($q) => $q->where('owner_user_id', $user->id))
            ->with(['lead:id,full_name,workflow_status'])
        ...
        $statusLabels = collect(GoodsMetaLeadWorkflow::statusOptions())->pluck('label', 'value')->all();

        $statusBreakdown = collect($byStatus)
            ->map(fn ($count, $status) => [
                'status' => $status,
        ...
        return [
            ...
            'today' => $todayLeads,
            'won' => $moutafaq,
            'hot' => (int) ($byStatus[GoodsMetaLead::WORKFLOW_SAKHIN] ?? 0),
            'warm' => (int) ($byStatus[GoodsMetaLead::WORKFLOW_DAFEE] ?? 0),
            'following' => (int) ($byStatus[GoodsMetaLead::WORKFLOW_MATAABA] ?? 0),
            'potential' => (int) ($byStatus[GoodsMetaLead::WORKFLOW_MOHTAMAL] ?? 0),
            'not_contacted' => $lamYattasel,
            'conversion_rate' => $closed > 0 ? round(($moutafaq / $closed) * 100, 1) : 0,
            'upcoming_calls_count' => $upcomingCallsCount,

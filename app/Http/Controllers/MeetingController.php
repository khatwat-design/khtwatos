...
class MeetingController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $isSalesTeam = $user->teams()->where('teams.id', 4)->exists();
        ...
        $query = Meeting::query()
            ->with([
                ...
                'goodsMetaLead:id,full_name,phone,workflow_status,campaign_name',
            ])
            ->orderByDesc('start_at');

        if ($isSalesTeam && ! $user->isAdmin()) {
            $goodsMetaLeadIds = GoodsMetaLead::query()
                ->where('owner_user_id', $user->id)
                ->pluck('id');
            $query->where(function ($q) use ($goodsMetaLeadIds) {
                $q->whereIn('goods_meta_lead_id', $goodsMetaLeadIds)
                    ->orWhereNull('goods_meta_lead_id');
            });
        }

        $this->meetings->applyVisibilityAndFilters($query, $request, $user, $hasArchiveColumn, $includeArchived);

        if ($request->filled('meta_lead_id')) {
            $query->where('goods_meta_lead_id', (int) $request->query('meta_lead_id'));
        }
        ...
        return Inertia::render('Meetings/Index', [
            ...
            'metaLeads' => $user->isAdmin()
                ? GoodsMetaLead::query()->select('id', 'full_name')->orderBy('full_name')->limit(500)->get()
                : GoodsMetaLead::query()
                    ->where('owner_user_id', $user->id)
                    ->select('id', 'full_name')
                    ->orderBy('full_name')
            ...
            'meetings' => $query->limit(200)->get()->map(fn (Meeting $m) => [
                ...
                'goods_meta_lead_id' => $m->goods_meta_lead_id,
                'meta_lead' => $m->goodsMetaLead ? [
                    'id' => $m->goodsMetaLead->id,
                    'name' => $m->goodsMetaLead->full_name,
                    'phone' => $m->goodsMetaLead->phone,
                    'status' => $m->goodsMetaLead->workflow_status,
                    'campaign' => $m->goodsMetaLead->campaign_name,
                ] : null,
            ]),
            'filters' => [
                ...
                'meta_lead_id' => $request->filled('meta_lead_id') ? (int) $request->query('meta_lead_id') : null,
            ],
    ...
    public function create(Request $request): Response
    {
        ...
        return Inertia::render('Meetings/Create', [
            'hosts' => $this->meetings->hostsForPicker($user),
            'clients' => $this->meetings->clientsForPicker($user),
            'metaLeads' => GoodsMetaLead::query()
                ->where('owner_user_id', $user->id)
                ->orWhereNull('owner_user_id')
                ->select('id', 'full_name', 'phone', 'campaign_name', 'workflow_status')
                ->orderBy('full_name')
                ->get(),
            'teams' => Team::query()->orderBy('sort_order')->get(['id', 'name', 'slug']),
            'defaults' => [
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $meeting = Meeting::query()->create([
            ...
            'goods_meta_lead_id' => $data['goods_meta_lead_id'] ?? null,
            'raw_payload' => null,
        ]);
    ...
    public function edit(Request $request, Meeting $meeting): Response
    {
        ...
        return Inertia::render('Meetings/Edit', [
            'meeting' => [
                ...
                'goods_meta_lead_id' => $meeting->goods_meta_lead_id,
    ...
    public function update(Request $request, Meeting $meeting): RedirectResponse
    {
        ...
        $meeting->update([
            ...
            'user_id' => $data['user_id'],
            'client_id' => $data['client_id'] ?? null,
            'goods_meta_lead_id' => $data['goods_meta_lead_id'] ?? $meeting->goods_meta_lead_id,
        ]);
    ...
    public function complete(Request $request, Meeting $meeting): RedirectResponse
    {
        ...
        if ($meeting->goods_meta_lead_id) {
            $lead = $meeting->goodsMetaLead;
            if ($lead) {
                $previous = $lead->workflow_status;
                $lead->workflow_status = GoodsMetaLead::WORKFLOW_MATAABA;
                $lead->workflow_status_managed_at = now();
                $lead->last_contact_date = now();
                $lead->save();

                if ($previous !== GoodsMetaLead::WORKFLOW_MATAABA) {
                    GoodsMetaLeadStatusHistory::query()->create([
                        'goods_meta_lead_id' => $lead->id,
                        'from_status' => $previous,
                        'to_status' => GoodsMetaLead::WORKFLOW_MATAABA,
                        'note' => 'إكمال اجتماع: '.($data['summary'] ? mb_substr($data['summary'], 0, 100) : 'بدون ملخص'),
    ...
    private function validatedMeeting(Request $request, bool $isUpdate = false): array
    {
        ...
        $rules = [
            'goods_meta_lead_id' => ['nullable', 'exists:goods_meta_leads,id'],
